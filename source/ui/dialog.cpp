...
Dialog::Dialog(int x_l, int x_r, int y_l, int y_r, std::string message, std::vector<std::string> button_strings) : message(message), button_strings(button_strings) {
	...
	message_height = Draw_get_height(message, 0.5) + 2 * MARGIN;
	message_width = Draw_get_width(message, 0.5) + 2 * MARGIN;
	for (auto &i : button_strings) button_height = std::max(button_height, Draw_get_height(i, 0.5) + 2 * MARGIN);
}
...
void Dialog::draw() {
	...
	// main message
	{
		float string_width = Draw_get_width(message, 0.5);
		float string_height = Draw_get_height(message, 0.5);
	...
	for (int i = 0; i < (int) button_strings.size(); i++) {
		...
		float string_width = Draw_get_width(button_strings[i], 0.5);
		float string_height = Draw_get_height(button_strings[i], 0.5);
